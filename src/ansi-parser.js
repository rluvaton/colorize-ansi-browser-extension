...
function parseAnsi(textInput) {
  ...
  class Code {
    ...
    get isBrightness() {
      return (
        this.value === Code.noBrightness ||
        this.value === Code.bright ||
        this.value === Code.dim
      );
    }
  }
 ...
 */
  ...
  const denormalizeBrightness = (s) =>
    s.replace(/(\u001b\[(1|2)m)/g, "\u001b[22m$1");
  const normalizeBrightness = (s) =>
    s.replace(/\u001b\[22m(\u001b\[(1|2)m)/g, "$1");
    ...
    return String(x)
      ...
      .map((line) =>
        denormalizeBrightness(
          open + replaceAll(normalizeBrightness(line), close, open) + close
        )
      )
      .join("\n");
  };
  ...
  const stringWrappingMethods = (() =>
    [
      ...
      ...[
        ...
        "BrightMagenta",
        "BrightCyan",
      ].map((k, i) => (!k ? [] : [["bg" + k, 100 + i, Code.noBgColor]])),
        ...
        !k
          ...
          : [
              ...
              [
                k,
                i,
                k === "bright" || k === "dim" ? Code.noBrightness : 20 + i,
              ],
            ]
      ),
    ...
    stringWrappingMethods.reduce(
      (memo, [k, open, close]) =>
        O.defineProperty(memo, k, {
          get: () =>
            assignStringWrappingAPI((str) =>
              wrapBefore(wrap(str, open, close))
            ),
        }),
